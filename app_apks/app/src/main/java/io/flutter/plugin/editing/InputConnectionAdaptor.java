...
public class InputConnectionAdaptor extends BaseInputConnection implements ListenableEditingState.EditingStateWatcher {
    ...
    public InputConnectionAdaptor(View view, int i10, TextInputChannel textInputChannel, ScribeChannel scribeChannel, KeyboardDelegate keyboardDelegate, ListenableEditingState listenableEditingState, EditorInfo editorInfo, FlutterJNI flutterJNI) {
        super(view, true);
    ...
    private static int clampIndexToEditable(int i10, Editable editable) {
        int iMax = Math.max(0, Math.min(editable.length(), i10));
        if (iMax != i10) {
            Log.d("flutter", "Text selection index was clamped (" + i10 + "->" + iMax + ") to remain in bounds. This may not be your fault, as some keyboards may select outside of bounds.");
        }
        return iMax;
    }
    ...
    private boolean handleHorizontalMovement(boolean z10, boolean z11) {
        ...
        int iMax = z10 ? Math.max(this.flutterTextUtils.getOffsetBefore(this.mEditable, selectionEnd), 0) : Math.min(this.flutterTextUtils.getOffsetAfter(this.mEditable, selectionEnd), this.mEditable.length());
                    ...
                    if (contentUri != null) {
                        try {
                            ...
                            if (inputStreamOpenInputStream != null) {
                                byte[] streamFully = readStreamFully(inputStreamOpenInputStream, ImageMetadata.CONTROL_AE_ANTIBANDING_MODE);
    ...
    public boolean performContextMenuAction(int i10) {
        beginBatchEdit();
        boolean zDoPerformContextMenuAction = doPerformContextMenuAction(i10);
        endBatchEdit();
    ...
    public InputConnectionAdaptor(View view, int i10, TextInputChannel textInputChannel, ScribeChannel scribeChannel, KeyboardDelegate keyboardDelegate, ListenableEditingState listenableEditingState, EditorInfo editorInfo) {
        this(view, i10, textInputChannel, scribeChannel, keyboardDelegate, listenableEditingState, editorInfo, new FlutterJNI());
    }
}
