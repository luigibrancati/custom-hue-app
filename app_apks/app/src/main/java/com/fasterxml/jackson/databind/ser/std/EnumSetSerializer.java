    ...
    public void serializeContents(EnumSet<? extends Enum<?>> enumSet, JsonGenerator jsonGenerator, SerializerProvider serializerProvider) {
        ...
        for (Enum<?> r12 : enumSet) {
            if (jsonSerializerFindContentValueSerializer == null) {
                jsonSerializerFindContentValueSerializer = serializerProvider.findContentValueSerializer(r12.getDeclaringClass(), this._property);
            }
            jsonSerializerFindContentValueSerializer.serialize(r12, jsonGenerator, serializerProvider);
        }
    }
